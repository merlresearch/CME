...
class WorkThread(threading.Thread):
    def __init__(self, queue, idx, gpu_id=None, logdir="."):
        """

        :param queue.Queue queue:
        :param idx:
        :param gpu_id: if gpu is not used, set None
        :param logdir:
        """
        ...
        if gpu_id is not None:
            ...
            self.environ["CUDA_VISIBLE_DEVICES"] = ""
...
def run_in_concurrent(list_args, concurrent=1, logdir=".", use_gpu=False, start_gpu_idx=0, available_gpus=None):
    ...
    for cur_idx in range(concurrent):
        ...
        thread = WorkThread(queue=args_queue, idx=cur_idx, gpu_id=gpu_id, logdir=logdir)
        thread.start()
